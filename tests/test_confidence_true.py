...
from bio_hansel.const import OK_NUM_TILES
...
def test_confidence_false(test_genome):
    ...
    # Here we can substitute whatever the results are for hansel based on the subtype
    ...
    # Where the real assertion is:
    assert st.reached_min_tiles == OK_NUM_TILES


    exp_cols = ['tilename', 'stitle', 'refposition', 'subtype',
       'is_pos_tile', 'sample', 'file_path', 'scheme', 'scheme_version', 'reached_min_tiles', 'is_confident']
